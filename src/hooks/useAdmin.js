...
export function useAdmin() {
  const [adminSettings, setAdminSettings] = useState({ hidden_news: [], custom_banners: [], news_sources: { pokemon: true, tcgnews: true, autogenerate: true } });
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        const settings = { hidden_news: [], custom_banners: [], tournaments_override: [], sponsored_ad: null, news_sources: { pokemon: true, tcgnews: true, autogenerate: true } };
        data.forEach(row => {
